...
A private housing inspection dataset from DGW is reserved exclusively for application testing. The dataset is not used for training, validation, or model optimization. Instead, it is used to assess practical applicability of the developed inspection system within realistic housing inspection workflows, and is therefore not used as a primary data source in this stage.

\subsection{Pipeline Validation}

Before full evaluation, validation checks are performed to verify dataset mappings and harmonization procedures. Sample images are manually inspected to confirm label consistency after harmonization. Output mappings from LLaVA to shared inspection categories are verified using controlled examples. Model failures and unmapped outputs are logged and analysed separately.

\subsection{Leakage Prevention}

The DGW dataset remains fully isolated from model development procedures. No DGW images are used for training, validation, hyperparameter optimization, or model selection. This separation reduces information leakage and strengthens practical applicability assessment.
...
Public datasets originate from different domains and collection procedures, potentially introducing differences in image quality, annotation consistency, class balance, and environmental conditions. Crack datasets, paint damage datasets, and housing inspection images may contain distributional differences that influence model behavior. Results should therefore be interpreted within the context of the selected datasets.
...
\node[evalblock, below=of harm] (eval) {Controlled Comparison\\
YOLOv8: Precision / Recall / mAP\\
Shared: Accuracy / Precision / Recall / F1\\
Confusion matrices + qualitative analysis};

\node[appblock, below=of eval] (app) {Application Testing\\
DGW housing archive\\
Practical housing workflow assessment\\
No training or validation use};
